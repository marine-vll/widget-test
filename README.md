...
`src/App.tsx` is a Kanban board: `useGrist<TaskRow, TaskMapped>()` reads every visible row (`w.records`) and groups them into one column per value of the mapped `Statut` Choice column (via `useGristSchema()` + `normalizeGristChoiceListEntries`, see `src/lib/grist-kanban.ts`). Drag-and-drop between columns (`@dnd-kit`) and the task side panel both write back through `w.table.update`/`w.table.create`/`w.table.destroy` + `w.mapBack(...)`. See `src/grist-types.ts` for typing patterns.
